...
import time
import ntptime
from md5 import digest # modified version of digest to return raw bytes not strings
...
def getSolis(solisInfo):
    ...
    Content_MD5 = base64.b64encode(digest(Body.encode('utf-8'))).decode('utf-8')
    encryptStr = (VERB + "\n"
        + Content_MD5 + "\n"
        + Content_Type + "\n"
        + Date + "\n"
        + CanonicalizedResource)
    h = hmac.new(solisInfo['solisSecret'].encode('utf-8'), msg=encryptStr.encode('utf-8'), digestmod=hashlib.sha1)
    ...
    print("header is: "+json.dumps(header))
    try:
# https://github.com/micropython/micropython-lib/issues/400 
# something to look into - probably need to use an ESP32
        resp = requests.post(req, data=Body, headers=header)
